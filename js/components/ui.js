...
function Card({children,style={},glow}){
  return <div className={`bg-surf border rounded-theme p-4 ${glow?'border-line-2 shadow-glow':'border-line'}`} style={style}>{children}</div>;
}
function Btn({onClick,children,color,style={},disabled,size='md'}){
  ...
  return <button onClick={onClick} disabled={disabled}
    className={`${pad} rounded-theme-sm font-semibold border border-line-2 transition-all duration-[120ms] ${disabled?'text-ink-3 opacity-45 cursor-default':'text-ink opacity-100 cursor-pointer'}`}
...
function Inp({value,onChange,placeholder,type='text',style={},onKeyDown}){
  return <input type={type} value={value} onChange={onChange} onKeyDown={onKeyDown} placeholder={placeholder}
    className="w-full px-[11px] py-2 bg-base border border-line-2 rounded-theme-sm text-ink text-[13px] outline-none box-border" style={style}/>;
}
function Txta({value,onChange,placeholder,rows=3,style={}}){
  return <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    className="w-full px-[11px] py-2 bg-base border border-line-2 rounded-theme-sm text-ink text-[12px] outline-none resize-y box-border" style={style}/>;
}
...
function Av({emoji,size=32,color}){
  return <div className="rounded-[50%] flex items-center justify-center shrink-0 border border-line-2"
